...
int main(int argc, char** argv){
    ...
    readTask.detach(); // 不分离 如果主线程没有调用join方法 就会造成tcp资源泄露
                       // //分离子线程 资源在线程结束后会自动被回收
    ...
    for(;;){
        ...
        switch (choice) {
            ...
            case 1:{ //login业务
                ...
                sem_wait(&rwsem); //主线程等待子线程处理登录的响应消息 等待子线程通知
            ...
            case 2: //register业务
            {
                ...
                sem_wait(&rwsem); //主线程等待子线程处理注册的响应消息 等待子线程通知
...
void doRegResponse(json& responsejs){
    if(responsejs["errno"].get<int>() != 0 ){
        std::cerr <<  "Invalid username, register error!" << std::endl;
    }
    else{ //注册成功
        std::cout << "Register success, userid is " << responsejs["id"] << ", keep it as identity" << std::endl;
    }
}
//处理登录响应逻辑
void doLoginResponse(json& responsejs){
    ...
    if (responsejs["errno"].get<int>() != 0) { // 登录失败
        ...
        if (responsejs.contains("offlinemsg")) {
            ...
            for (std::string &msg :
                msgs) { // 从离线消息中判断是个人离线消息还是群组离线消息
                json js = json::parse(msg);
                int msgtype = js["msgid"].get<int>();
                std::cout << "type: " << msgtype;
                if (msgtype == ONE_CHAT_MSG) {
                    std::cout << "personal message: " << js["time"].get<std::string>()
                            << " [" << js["id"] << "] "
                            << js["name"].get<std::string>()
                            << " said: " << js["msg"].get<std::string>() << std::endl;
                } else if (msgtype == GROUP_CHAT_MSG) {
                    std::cout << "group [id:" << js["groupid"] << "]"
                            << "message: " << js["time"].get<std::string>() << " ["
